...
def enhanced_google_search(query, reasoning_context, google_subscription_key, 
    ...
    HyDE is applied AFTER Google search to re-rank results based on hypothetical documents.
    ...
    Returns:
        Search results
    """
    # First, get the regular Google search results
    initial_results = google_web_search(query, google_subscription_key)
    
    if not use_hyde or 'items' not in initial_results:
    ...
    search_results = initial_results['items']
    
    # Create embeddings for search result snippets
    result_texts = []
    ...
    # Calculate similarity scores between each result and the weighted HyDE embedding
    similarity_scores = []
    for embedding in result_embeddings:
        # Normalize the embedding
        normalized_embedding = embedding / np.linalg.norm(embedding)
        # Calculate cosine similarity
        similarity = np.dot(weighted_embedding, normalized_embedding)
        similarity_scores.append(similarity)
    
    # Create (result, score) pairs for sorting
    result_score_pairs = list(zip(search_results, similarity_scores))
    
    # Sort results by similarity score in descending order
    sorted_results = [result for result, _ in sorted(
        result_score_pairs, key=lambda x: x[1], reverse=True)]
    
    # Return the top_k re-ranked results
    re_ranked_results = {
        'items': sorted_results[:top_k]
    }
