...
def enhanced_google_search(query, reasoning_context, google_subscription_key, 
    ...
    """
    if not use_hyde:
        # Get query embedding for reranking even without HyDE
        query_embedding = embedding_model.encode(query)
        query_embedding = query_embedding / np.linalg.norm(query_embedding)
        results = google_web_search(query, google_subscription_key)
    else:
        ...
        query_embedding = weighted_embedding
        ...
        # Store in the format expected by downstream processing
        results = {'items': unique_results}
    
    # Implement cosine similarity reranking
    reranked_results = rerank_by_cosine_similarity(results, query_embedding, embedding_model)
    
    # Format results to match the expected structure for Google Search API
    combined_results = {
        'items': reranked_results[:top_k]  # Limit to top_k results
    }
    
    return combined_results

def rerank_by_cosine_similarity(results, query_embedding, embedding_model):
    """
    Rerank search results using cosine similarity between query and document embeddings.
    
    Args:
        results: Search results from Google API
    ...
    # Combine results with similarity scores and sort
    scored_results = list(zip(results['items'], similarity_scores))
    scored_results.sort(key=lambda x: x[1], reverse=True)
    
    print(f"Reranked {len(scored_results)} documents by cosine similarity")
    
    # Extract reranked results
    reranked_items = [item for item, score in scored_results]
